...
void timestep(double mass[NMAX], double pos[NMAX][3], double vel[NMAX][3], int N,double *dt,
        double r[NMAX], double vrdot[NMAX], double v2[NMAX]) {
    double stepi;
    stepi = 1.e38;
    for (int j=0; j<(N-1); j++) {
        ...
        double vdotdr2 = vrdot[j] / (r[j] * r[j]);

        //double vdotdr2 = (dx*dvx+dy*dvy+dz*dvz) / r2;

        double tau = ETA_SYM / M_SQRT2 * r[j] * sqrt(r[j]/(mass[N-1]+mass[j]));
        ...
        stepi = min(tau, stepi);
        tau = ETA_SYM * r[j] / sqrt(v2[j]);
        dtau = tau * vdotdr2 * (1 + (mass[N-1]+mass[j]) / (v2[j] * r[j]));
        dtau = (dtau < 1)? dtau: 1;
...
void force_and_jerk(double pos[NMAX][3], double vel[NMAX][3], double mass[NMAX], int N,double *tpos, double *tvel,
        double *ac, double *jc, double r[NMAX], double vrdot[NMAX], double v2[NMAX]) {
...
void initial(double pos[NMAX][3],double vel[NMAX][3], double mass[NMAX],int N,double *ac, double *jc, double *dt)  {
    ...
    double r[NMAX], vrdot[NMAX], v2[NMAX];
    force_and_jerk(pos,vel,mass,N,tpos,tvel,ac,jc,r,vrdot,v2);
    timestep(mass,pos,vel,N,dt,r,vrdot,v2);
}
...
void evolve_hermite(double pos[NMAX][3], double vel[NMAX][3], double mass[NMAX],int N,double *ac, double *jc, double *dt) {
    ...
    force_and_jerk(pos,vel,mass,N,tpos,tvel,aci,jci,r,vrdot,v2);
    corrector(ac,jc,tpos,tvel,tposc,tvelc,aci,jci,a2ci,a3ci,*dt);
    
    force_and_jerk(pos,vel,mass,N,tposc,tvelc,aci,jci,r,vrdot,v2);
    corrector(ac,jc,tpos,tvel,tposc,tvelc,aci,jci,a2ci,a3ci,*dt);
...
int evolve_system(double pos[NMAX][3], double vel[NMAX][3], double mass[NMAX], double thresh[NMAX],int N,double *ac, double *jc, double *dt, double *t, double t_end) {
    ...
    while(t_cur < t_end) {
        ...
        evolve_hermite(pos,vel,mass,N,ac,jc,dt);
        //debug_print(pos,vel);
